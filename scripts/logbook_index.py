...
def build_daily_section() -> str:
    ...
    for path in daily_files:
        date = path.stem
        lines.append(f"- {date} — [{date}]({{{{ '/daily/{path.stem}' | relative_url }}}})")
    if len(lines) == 1:
        lines.append("- (none)")
    return "\n".join(lines)
...
def build_adr_section() -> str:
    ...
    if not adr_metadata:
        ...
        for item in adr_metadata:
            title_link = (
                f"<a href=\"{{{{ '/decisions/{Path(item['path']).stem}' | relative_url }}}}\">"
                f"{item['title']}</a>"
            )
            lines.append(
...
def build_simple_section(title: str, directory: Path, label: str) -> str:
    ...
    lines = [title]
    for path in files:
        lines.append(f"- [{path.stem}]({{{{ '/{label}/{path.stem}' | relative_url }}}})")
    if len(lines) == 1:
        lines.append("- (none)")
    return "\n".join(lines)
